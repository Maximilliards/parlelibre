...
export function ContactPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('messages').insert({
      titre: `Contact — ${pseudonym || 'Anonyme'}`,
      categorie: 'autre',
      contenu_original: `Message de contact\nPseudonyme: ${pseudonym || 'Anonyme'} (${email})\n\n${message}`,
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-contact-email`, {
        ...
        body: JSON.stringify({ name: pseudonym, email, message }),
      });

      if (!response.ok) {
        setSending(false);
        setError("Votre message a bien été enregistré, mais l’envoi de l’email a échoué. Veuillez réessayer.");
        return;
      }
    } catch {
      setSending(false);
        ...
        <div className="md:col-span-3">
          {sent ? (
            ...
            <form onSubmit={onSubmit} className="card p-6 sm:p-8 space-y-5">
              <div>
                <label className="label" htmlFor="pseudonym">Votre pseudonyme (optionnel)</label>
                <input
                  id="pseudonym"
                  className="input"
                  value={pseudonym}
