...
export function BookingPage() {
  ...
  const handlePayment = async () => {
    ...
    await supabase.from('notifications').insert({
      type: 'new_booking',
      message: `Nouvelle réservation : ${name.trim()} le ${selected.date.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })} à ${selected.start}`,
      priority: 'normal',
    });
    await supabase.from('notifications').insert({
    ...
    <div className="grid gap-8 lg:grid-cols-3">
      <div className="lg:col-span-2">
        <form onSubmit={onSubmit} className="card p-6 sm:p-8 space-y-5">
          ...
          <div>
            <label className="label" htmlFor="bk-name">Nom complet</label>
            <input id="bk-name" className="input" value={name} onChange={(e) => setName(e.target.value)} required placeholder="Votre nom" />
          </div>
          <div>
          ...
          <div>
            ...
            <input id="bk-phone" type="tel" className="input" value={phone} onChange={(e) => setPhone(e.target.value)} required placeholder="+225 ..." />
...
function PaymentStep({
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-3">
      ...
      <BookingSummary selected={selected} client={{ name, email, phone }} />
    </div>
  );
}
    ...
    <div className="max-w-2xl mx-auto">
      <div className="card p-8 sm:p-10 text-center animate-scale-in">
        ...
        <p className="mt-3 text-stone-600">
          Merci {name || ''}, votre séance d'écoute est réservée. Une confirmation a été envoyée.
        </p>
